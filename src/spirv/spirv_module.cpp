...
namespace dxvk {
  ...
  uint32_t SpirvModule::defConstCached(
    ...
    const uint32_t*               argIds) {
    if (!cache)
      cache = this->defConstUnique(op, typeId, argCount, argIds);
    return cache;
  }
